package com.dnabuba.tacademy.abuband.Band;
...
public class BandAddActivity extends AppCompatActivity {
        ...
        btn_send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(edit_bandSerial.getText().toString())) {
                    BandSerialCheck(edit_bandSerial.getText().toString());
                } else {
    ...
    //시리얼 체크
    private void BandSerialCheck(String serial) {
        NetworkManager.getInstance().getBandSerial(this, serial, new NetworkManager.OnResultListener<NetworkCodeResult>() {
            @Override
            public void onSuccess(NetworkCodeResult result) {
                    switch (result.code)    {
                        case RESULT_CODE_0:
